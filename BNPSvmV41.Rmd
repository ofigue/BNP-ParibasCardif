...
for(i in 1:k){
  ...
  predSVMtest=predict(SVMModel,newdata=test[,!(names(cv.test) %in% c("target"))],
                 probability = TRUE)
  #attr(predSVMtest, "probabilities")[,1] #gives the probability of 1 and zero
  ensemble <- ensemble + attr(predSVMtest, "probabilities")[,1] #gives probability of 1 and 0
...
write.csv(submission, "predSVMtest.csv", row.names = FALSE)
